...
- 新增本地结构化 JSON 比较，支持粘贴文本、文件和源记录，展开字符串化内容，按路径展示差异并忽略指定字段。
- 新增记录表格，支持将嵌套字段设为列、组合条件筛选全部记录、保留源行号并导出 CSV。
- 新增字段体检，统计出现率、缺失值、null、空字符串和类型分布，点击计数可筛出对应记录。
- 新增问题片段报告，支持选择源行号、按指定路径脱敏、预览结果并下载 Markdown 或 JSONL。

### Changed

- 菜单与输出标签增加平滑的悬停反馈，同时保留键盘导航和减少动态效果的行为。
- 字体改为本地打包，带版本标识的 Web 静态资源使用长期缓存，减少外部字体请求和重复下载。
- 更新 UI 与构建依赖，采用 pnpm 12 和更严格的维护检查，并保留 React 19.2 以满足现有 JavaScript 包体预算。
- Web 与浏览器扩展应用版本（包括 Safari 宿主应用营销版本）升级到 `1.2.3`。

### Fixed

- 浏览器扩展导入选中文本失败时提供恢复指引，并清理已消费的导入参数，避免刷新页面后重复提示。
- 通过受限读取、后台解析和有上限的记录缓存，减少大型本地记录在复制、导出和重复查看时的内存占用，并保持浏览器输入响应。
- 记录表格会缩小超大的读取批次，并在格式化单元格前筛选记录，避免已排除的大值阻止匹配结果展示。
- 问题片段报告逐条处理本地记录，并在脱敏过程中让出执行时间，降低内存峰值和长时间停顿。
- 无效 JSONL 行只保留有上限的错误预览；复制与导出要求完整源文本，避免静默输出被截断的数据。
- 较长的 JSON 比较值会展示首个变化字符附近的上下文，让初始预览范围之外的差异也能被看见。

## [1.2.2] - 2026-09-05

### Added

- Web 应用新增英文、简体中文与日文的多语言产品更新页面，并在状态栏提供直接查看入口。
- 导入面板与空状态区域增加适配当前明暗主题的背景装饰。

### Changed

- 界面图标统一迁移至 Phosphor 图标库，规范整体视觉层级。
- 采用 `mise` 统一管理开发环境运行时，并将 Vitest 升级至版本 5，解耦发布门禁检查对硬编码版本的依赖。
- 收敛内部架构边界：按模块细化指导文档，清理未使用的路径查询与展开辅助函数，并移除多余的解析文本转发层。
- Web 与浏览器扩展应用版本（包括 Safari 宿主应用营销版本）升级到 `1.2.2`。

### Fixed

- 内存缓冲导出增加显式字节上限与本地化提示，防止大文件导出时内存溢出导致浏览器标签页崩溃。
- 从本地文件复制未水合记录时在解析前施加安全边界，避免超大行引发无界内存分配。
- 全量记录的 Web Worker 解析与搜索请求改为串行化调度，避免后台线程争用与任务积压。
- 修复初次载入 Agent 会话时对初始输入的重复解析，改为只解析一次，加快首次渲染速度并降低内存开销。
- 下拉单选菜单在用户选择选项后自动关闭。
