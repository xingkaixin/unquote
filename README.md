...
- 问题片段导出：按源行号选择记录，按路径脱敏，预览并下载 Markdown 报告或 JSONL
- 字段体检：统计所选字段的出现率、缺失、null、空字符串和类型分布，点击计数筛出对应记录
- 记录表格：选取嵌套字段作为列，以组合条件筛选全部记录，保留源行号并导出 CSV
- 本地结构化 JSON 比较：支持粘贴、文件与源记录，展开字符串化内容，按路径显示增删改并忽略指定字段
