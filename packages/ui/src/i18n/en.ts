export const en = {
  "table.title": "Record table",
  "table.description":
    "Choose up to 12 JSON paths as columns. All conditions must match. Scans all source records, including nested stringified fields; failed JSON lines are counted separately.",
  "table.column": "Column {count}",
  "table.path": "Field path",
  "table.condition": "Condition",
  "table.value": "Value",
  "table.any": "Any value",
  "table.equals": "Equals",
  "table.contains": "Text contains",
  "table.greater": "Number greater than",
  "table.less": "Number less than",
  "table.missing": "Missing field",
  "table.remove": "Remove column",
  "table.add": "Add column",
  "table.scan": "Apply and scan",
  "table.cancel": "Cancel",
  "table.export": "Export CSV",
  "table.progress": "Scanned {count} / {total} records",
  "table.summary": "{count} matches · {total} scanned · {failed} invalid lines skipped",
  "table.csvHint":
    "Long cells show previews; CSV contains complete values. Missing fields and empty strings export as empty cells. Formula-like strings are prefixed with an apostrophe. Import numeric columns as text in spreadsheets to preserve precision.",
  "table.limit":
    "Table exceeds its processing limit (12 columns, 100,000 matches, 20 MiB results, 64 KiB per cell or 4 MiB per read batch). Narrow the columns or source. No partial result was exported.",
  "table.failed":
    "Could not scan. Check field paths and numeric conditions, or choose a smaller source.",
  "table.line": "Source line",
  "table.previous": "Previous",
  "table.next": "Next",
  "diff.title": "Compare JSON",
  "diff.description":
    "Compare expanded JSON locally. Object key order is ignored; arrays use positions and numbers retain source spelling. Up to 512 KiB per input; long values show a preview.",
  "diff.before": "Before",
  "diff.after": "After",
  "diff.file": "Open JSON file",
  "diff.line": "Source line",
  "diff.loadRecord": "Load record",
  "diff.ignore": "Ignore paths, one per line (including descendants)",
  "diff.compare": "Compare",
  "diff.comparing": "Comparing…",
  "diff.limit":
    "Comparison exceeds its size or complexity limit. Choose smaller inputs or records.",
  "diff.failed": "Could not compare. Check both JSON inputs, source lines and ignored paths.",
  "diff.count": "{count} differences",
  "diff.equal": "No differences under these comparison rules.",
  "diff.path": "Path / change",
  "diff.added": "Added",
  "diff.removed": "Removed",
  "diff.changed": "Value changed",
  "diff.type": "Type changed",
  "diff.previous": "Previous",
  "diff.next": "Next",

  "app.tab.agent": "Agent",
  "app.tab.trajectory": "Trajectory",
  "app.changelog": "Product updates",
  "app.chrome": "Chrome Extension",
  "app.edge": "Edge Extension",
  "app.skipToContent": "Skip to main content",
  "deferredLoad.title": "This view couldn't be loaded",
  "deferredLoad.description":
    "The application code may have changed, or the network may be unavailable.",
  "deferredLoad.retry": "Try again",
  "theme.toggle": "Switch theme",
  "theme.light": "Light",
  "theme.dark": "Dark",
  "theme.system": "System",
  "locale.toggle": "Change language",
  "locale.english": "English",
  "locale.chinese": "Chinese (Simplified)",
  "locale.japanese": "Japanese",
  "toolbar.copyJsonl": "Copy JSONL",
  "toolbar.copyFormattedJson": "Copy formatted JSON",
  "toolbar.copyBlocked": "Data is too large to copy — use Export instead",
  "copy.failed": "Copy failed",
  "toolbar.exporting": "Exporting…",
  "toolbar.exportDone": "File exported",
  "toolbar.exportTooLarge": "Export exceeds 64 MiB. Filter fewer records and try again.",
  "toolbar.exportFailed": "Export failed",
  "toolbar.export": "Export",
  "toolbar.exportJsonl": "Export JSONL",
  "toolbar.exportJson": "Export JSON",
  "toolbar.expandAll": "Expand All",
  "toolbar.collapseAll": "Collapse All",
  "command.open": "Open command panel",
  "command.openShort": "Commands",
  "command.palette": "Find, jump, and commands",
  "command.placeholder": "Search text, or enter $.path to jump...",
  "command.close": "Close command panel",
  "command.search": "Search",
  "command.searchMode": "Find",
  "command.pathMode": "Path",
  "command.searchMatches": "{count} matches",
  "command.pathMatches": "{count} paths",
  "command.visibleRecords": "{shown}/{total} records visible",
  "command.filterCommands": "Filter commands...",
  "command.recordFilters": "Record filters",
  "command.active": "active",
  "agent.metric.events": "Events",
  "agent.metric.messages": "Messages",
  "agent.metric.turns": "Turns",
  "agent.metric.tools": "Tools",
  "agent.sessionId": "Session",
  "agent.model": "Model",
  "agent.warnings": "{count} warnings",
  "agent.warning.title": "Parsing warnings",
  "agent.warning.invalidJson": "Invalid JSON",
  "agent.warning.projectionFailed": "Could not interpret Agent data in this Record",
  "agent.warning.more": "{count} more warnings not shown",
  "agent.warning.openRecord": "Open Record",
  "agent.timeline": "Timeline",
  "agent.overview": "Session overview",
  "agent.openInJsonl": "View in JSONL",
  "agent.toolOutput": "{tool} → output",
  "agent.status.pending": "Running",
  "agent.status.completed": "Done",
  "agent.status.failed": "Failed",
  "agent.conversation": "Conversation",
  "agent.noConversation": "No conversation items in this session",
  "agent.line": "Line {line}",
  "agent.turn": "Turn {turn}",
  "agent.role.user": "User",
  "agent.role.assistant": "Assistant",
  "agent.role.system": "System",
  "agent.role.thinking": "Thinking",
  "agent.role.toolCall": "Tool call",
  "agent.role.toolResult": "Tool result",
  "agent.category.user": "user",
  "agent.category.assistant": "assistant",
  "agent.category.thinking": "thinking",
  "agent.category.tool": "tool",
  "agent.category.system": "system",
  "agent.category.meta": "meta",
  "agent.category.unknown": "unknown",
  "trajectory.title": "Trajectory",
  "trajectory.summary": "Session trajectory",
  "trajectory.overview": "Overview",
  "trajectory.ledger": "Event ledger",
  "trajectory.detail": "Detail",
  "trajectory.detailEmpty": "Select an item to inspect",
  "trajectory.unassigned": "Unassigned",
  "trajectory.empty": "No trajectory items",
  "trajectory.metric.turns": "Turns",
  "trajectory.metric.events": "Events",
  "trajectory.metric.tools": "Tools",
  "trajectory.metric.failures": "Failures",
  "trajectory.metric.tokens": "Tokens",
  "trajectory.metric.duration": "Duration",
  "trajectory.token.input": "Input",
  "trajectory.token.output": "Output",
  "trajectory.token.cacheRead": "Cache read",
  "trajectory.token.cacheWrite": "Cache write",
  "trajectory.token.reasoning": "Reasoning",
  "trajectory.lane.activity": "Activity",
  "trajectory.lane.model": "Model",
  "trajectory.lane.tool": "Tool",
  "trajectory.zoomIn": "Zoom in",
  "trajectory.zoomOut": "Zoom out",
  "trajectory.reset": "Reset",
  "trajectory.rangeStart": "Range start",
  "trajectory.rangeEnd": "Range end",
  "trajectory.noTimeline": "No timeline data",
  "trajectory.idleGap": "Idle {duration}",
  "trajectory.search": "Search trajectory",
  "trajectory.searchPlaceholder": "Search events…",
  "trajectory.kind": "Kind",
  "trajectory.statusFilter": "Status",
  "trajectory.filterFailures": "Show only failed items",
  "trajectory.clearFilters": "Clear filters",
  "trajectory.visibleCount": "{visible} of {total} visible",
  "trajectory.kind.all": "All",
  "trajectory.kind.user": "User",
  "trajectory.kind.system": "System",
  "trajectory.kind.assistant": "Assistant",
  "trajectory.kind.reasoning": "Reasoning",
  "trajectory.kind.tool": "Tool",
  "trajectory.kind.subagent": "Subagent",
  "trajectory.kind.compaction": "Compaction",
  "trajectory.status.completed": "Completed",
  "trajectory.status.running": "Running",
  "trajectory.status.failed": "Failed",
  "trajectory.status.aborted": "Aborted",
  "trajectory.derivedStep": "≈ derived step {step}",
  "trajectory.derivedStepHint": "Derived from available session data; not a source step.",
  "trajectory.line": "Line {line}",
  "trajectory.turn": "Turn {turn}",
  "trajectory.turnUnindexed": "Turn",
  "trajectory.itemCount": "{count} events",
  "trajectory.time": "Time",
  "trajectory.duration": "Duration",
  "trajectory.startedAt": "Started",
  "trajectory.endedAt": "Ended",
  "trajectory.callId": "Call ID",
  "trajectory.warnings": "Warnings",
  "trajectory.raw": "Raw JSON",
  "trajectory.rawCall": "Call input",
  "trajectory.rawResult": "Result output",
  "trajectory.rawUnavailable": "Content not fully loaded — open the Record to view it.",
  "trajectory.rawTruncated": "Truncated preview; open the Record for the full content.",
  "trajectory.openRecord": "Open Record",
  "trajectory.openCall": "Open call Record",
  "trajectory.openResult": "Open result Record",
  "trajectory.openCompletion": "Open completion Record",
  "trajectory.warning.missingTimestamp": "Missing timestamp",
  "trajectory.warning.missingTurnStart": "Missing turn start",
  "trajectory.warning.reversedTimestamp": "Timestamp is out of order",
  "trajectory.warning.unpairedCall": "Unpaired tool call",
  "trajectory.warning.unpairedResult": "Unpaired tool result",
  "trajectory.warning.unpairedCompletion": "Unpaired tool completion",
  "trajectory.warning.duplicateCall": "Duplicate tool call",
  "trajectory.warning.duplicateResult": "Duplicate tool result",
  "trajectory.warning.duplicateCompletion": "Duplicate tool completion",
  "trajectory.warning.openTurn": "Turn still open",
  "trajectory.warning.unattachedTokens": "Unattached token usage",
  "input.sourceLabel": "Source input",
  "input.modeLabel": "Input format",
  "input.mode.auto": "Auto",
  "input.mode.json": "JSON",
  "input.mode.jsonl": "JSONL",
  "input.placeholder": "Paste JSON / JSONL, or drop a file here.",
  "input.dropIdle": "Paste · Drop · Choose file",
  "input.dropRelease": "Release to parse",
  "input.acceptedTypes": ".json · .jsonl",
  "input.chooseFile": "Choose file…",
  "input.detectWaiting": "Waiting for input",
  "input.detectJson": "Detected JSON",
  "input.detectJsonl": "Detected JSONL · {lines} lines",
  "input.detectJsonlAtLeast": "Detected JSONL · at least {lines} lines",
  "input.detectInvalid": "Cannot parse — check the format",
  "input.readingFile": "Reading {name} · {size}",
  "input.parsingFile": "Parsing {name} · {size} · {processed} lines processed",
  "input.loadedFile": "{name} · {size} · {processed} lines processed",
  "input.readFailed": "Failed to read file",
  "input.fileTooLargeForMemory": "File is too large to load as JSON — choose JSONL to stream it",
  "input.tooLargeWithoutWorker": "This input is too large to parse without a background worker",
  "samples.label": "Samples",
  "samples.ariaLabel": "Sample inputs",
  "samples.escapedApiResponse": "Escaped API response",
  "samples.agentToolCallJsonl": "Agent tool-call JSONL",
  "samples.codexRolloutJsonl": "Codex rollout JSONL",
  "samples.mixedValidInvalidJsonl": "Mixed valid/invalid JSONL",
  "empty.eyebrow": "START",
  "empty.headline": "Paste, drop, or choose a file",
  "empty.subtitle":
    "Unquote detects JSON and JSONL, expands the escaped JSON strings inside, and builds a readable timeline for agent sessions. Everything is parsed locally in your browser.",
  "import.title": "Import data",
  "import.subtitle": "Paste text, choose a file, or drop one in",
  "import.back": "Back",
  "import.parse": "Parse",
  "import.formatLabel": "Format",
  "import.formatHint": "Auto mode detects JSONL line by line and expands stringified JSON",
  "source.change": "Change",
  "source.pasted": "Pasted text",
  "source.none": "No data loaded",
  "status.maxDepth": "max depth {depth}",
  "status.expandedNested": "{count} nested expanded",
  "status.failed": "{count} failed",
  "status.empty": "No data loaded · waiting for import",
  "status.clear": "Clear",
  "status.hintPath": "Enter jump to path",
  "status.hintPalette": "⌘K command palette",
  "rail.line": "Line {line}",
  "rail.turn": "Turn {turn}",
  "rail.records": "Records",
  "rail.empty": "No records match this filter",
  "tree.empty": "Select a record",
  "tree.copyRecord": "Copy record",
  "inspector.title": "Selected node",
  "inspector.empty": "Select a node in the tree",
  "inspector.loading": "Loading value…",
  "inspector.truncated": "Value truncated for preview",
  "inspector.tooLarge": "This value is too large to preview",
  "inspector.copyBlocked": "This value is too large to copy",
  "inspector.copyValue": "Copy value",
  "inspector.copyPath": "Copy path",
  "inspector.nestedTitle": "This record contains stringified JSON",
  "inspector.nestedBody":
    "Some fields hold escaped JSON text. Expand them to search and jump inside their keys and values.",
  "inspector.expandNested": "Expand all",
  "stats.label": "{total} total · {success} ok · {failed} err",
  "stats.filteredLabel": "{shown}/{total} shown · {success} ok · {failed} err",
  "stats.progress": "{processed} lines · {elapsed} ms",
  "error.parseFailed": "Parse failed",
  "error.location": "Line {line}, column {column}",
  "error.rawLine": "Raw line",
  "error.context": "Context",
  "error.copyRawLine": "Copy raw line",
  "error.copyDetails": "Copy error",
  "error.message": "Error: {message}",
  "search.inputLabel": "Search or jump",
  "search.regex": "Regex",
  "search.caseSensitive": "Case sensitive",
  "search.prev": "Previous match",
  "search.next": "Next match",
  "search.clear": "Clear search",
  "search.jq": "JSONPath / jq syntax",
  "search.timeout": "Search timed out",
  "search.failed": "Search failed",
  "search.tooLargeWithoutWorker": "Too large to search without a background worker",
  "search.regexRequiresWorker": "Regex search requires a background worker",
  "filter.all": "All",
  "filter.matches": "Matches",
  "filter.errors": "Errors",
  "filter.nested": "Nested",
  "filter.nestedTopLevel": "Top-level nested",
  "filter.tools": "Tools",
  "filter.messages": "Messages",
  "filter.events": "Events",
  "filter.hint": "{shown} / {total} records match this filter",
  "insight.kind.error": "Error",
  "insight.kind.tool": "Tool",
  "insight.kind.message": "Message",
  "insight.kind.event": "Event",
  "path.jump": "Jump to path",
  "path.invalid": "Invalid path syntax",
  "path.notFound": "No path match",
} as const;

// en is the canonical schema: the key set and Messages type derive from it, so
// every locale catalog stays in sync without a separate interface.
export type Messages = Record<keyof typeof en, string>;
export type MessageKey = keyof typeof en;
