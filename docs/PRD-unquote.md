...
以下仅是可在明确用户需求和成本后评估的候选，不构成产品路线承诺：更专业的文本编辑体验和 JSON Schema 校验。
...
导出菜单提供问题片段入口，按源行号或范围选择记录，允许添加备注和逐条应用的明确脱敏路径。路径替换整个值，覆盖展开后的字符串化 JSON。预览与 Markdown / JSONL 下载共用一次脱敏结果；修改选择、备注或规则后必须重新生成。备注和未指定字段不自动脱敏。报告保留源行号，不包含原始转义副本。无效行或缺失行明确拒绝，用户手动选择相关工具调用与结果的行。最多 1000 条记录、8 MiB 内容、每条 5 万节点；超限和不完整节点拒绝生成。
...
顶部“比较 JSON”支持两份粘贴文本、本地 JSON 文件或按源行号载入当前记录。比较展开后的结构，对象忽略键顺序，数组按位置比较，数字按保留的源字面量比较；忽略路径同时忽略其子树。输入每份最多 512 KiB，比较最多访问 50,000 个节点、输出少于 5,000 处差异，超限明确失败。结果每页 50 项，长值只展示预览。比较不修改当前 Source。
