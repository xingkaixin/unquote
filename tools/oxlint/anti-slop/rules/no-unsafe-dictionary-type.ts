...
export const noUnsafeDictionaryTypeRule = defineRule({
	meta: {
		...
		schema: [{
			type: "object",
			properties: { allowUnknownValues: { type: "boolean" } },
			additionalProperties: false,
		}],
		defaultOptions: [{ allowUnknownValues: false }],
	},
	createOnce(context) {
		let environment: TypeEnvironment | null = null;
		const allowsUnknownValues = () => {
			const option = context.options?.[0];
			return typeof option === "object" && option !== null &&
				"allowUnknownValues" in option && option.allowUnknownValues === true;
		};
		const report = (node: ESTree.Node, value: string) => {
			if (value === "unknown" && allowsUnknownValues()) return;
			context.report({ node, messageId: "unsafeDictionary", data: { value } });
		};
		const reportIfUnsafe = (node: ESTree.TSType) => {
			if (environment === null || !shouldReportType(node, environment)) return;
			const unsafe = classifyUnsafeDictionary(node, environment, allowsUnknownValues());
